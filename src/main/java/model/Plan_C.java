 ...
 * @author eljos
 */
public class Plan_C extends PlanEstudio{
    


    public Plan_C(Carrera carrera, char tipo) {
        super(carrera, tipo);
        this.setDescripcion("Aprobar las Cursadas de todas las Materias Correlativas y los Finales de los 5 Cuatrimestres Previos a la materia que se quiere anotar");
    }
    
    
    private boolean verificarFinales5CuatrimestresPrevios(int nivelMateria, Alumno alumno){
        ...
        if(this.getCarrera().getMaterias().size()>0){
            if(nivelMateria == 15){
                    ...
                    for(Materia materia : this.getCarrera().getMaterias()){
                        if(materia.getNivel()<15 && materia.getNivel() > 1){
                ...
                if(nivelMateria == 20){
                    ...
                    for(Materia materia : this.getCarrera().getMaterias()){
                        if(materia.getNivel()<20 && materia.getNivel() > 2){
                            materiasAVerificarFinales.add(materia);
                        }
                    }
                    ...
                    if(nivelMateria == 24){
                        ...
                        for(Materia materia : this.getCarrera().getMaterias()){
                            if(materia.getNivel()<24 && materia.getNivel() > 3){
                                materiasAVerificarFinales.add(materia);
                            }
                        }
                        ...
                        if(nivelMateria == 30){
                            ...
                            for(Materia materia : this.getCarrera().getMaterias()){
                                if(materia.getNivel()<30 && materia.getNivel() > 6){
                                    materiasAVerificarFinales.add(materia);
                                }
                            }
                            ...
                            for(Materia materia : this.getCarrera().getMaterias()){
                                if(materia.getNivel()<35 && materia.getNivel() > 8 ){
                                    materiasAVerificarFinales.add(materia);
                                }
                            }
    ...
    public boolean verificarPlan(Alumno alumno, Materia materia) {
        ...
        if(nivelMateria<15){
            ...
            condicion2 = verificarFinales5CuatrimestresPrevios(nivelMateria, alumno);
        }
